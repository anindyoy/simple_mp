    ...
    <div class="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            {{ $product->condition === 'baru' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' }}">
                ...
                <div class="mt-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3">
                    <div class="flex items-center gap-2">
                        @if (auth()->check() && (auth()->user()->is_admin && config('app.demo_mode') || (int) $product->lapak?->user_id === (int) auth()->id()))
                            <a href="{{ route('filament.admin.resources.products.edit', $product) }}"
                                target="_blank"
                                class="inline-flex items-center justify-center gap-1 px-1.5 py-1.5 sm:px-3 sm:py-2.5 text-xs sm:text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg shadow-sm transition-all active:scale-95 whitespace-nowrap">
                        ...
                        <button onclick="copyProductLink()"
                            class="inline-flex items-center justify-center gap-1 px-1.5 py-1.5 sm:px-3 sm:py-2.5 text-xs sm:text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-sm transition-all active:scale-95 whitespace-nowrap">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @if ($hasReported)
                        <span class="inline-flex items-center justify-center gap-1.5 text-xs sm:text-sm text-gray-400 cursor-not-allowed whitespace-nowrap">
                            <svg class="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4.5c-.77-.833-2.694-.833-3.464 0L3.34 16.5c-.77.833.192 2.5 1.732 2.5z" />
                            </svg>
                            <span>Anda sudah melaporkan ini</span>
                        </span>
                    @else
                        <button
                            onclick="document.getElementById('reportProductModal').classList.remove('hidden')"
                            class="inline-flex items-center justify-center gap-1 px-1.5 py-1.5 sm:px-3 sm:py-2.5 text-xs sm:text-sm font-semibold text-red-600 bg-red-50 hover:bg-red-100 border border-red-200 rounded-lg shadow-sm transition-all active:scale-95 whitespace-nowrap">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4.5c-.77-.833-2.694-.833-3.464 0L3.34 16.5c-.77.833.192 2.5 1.732 2.5z" />
                            </svg>
