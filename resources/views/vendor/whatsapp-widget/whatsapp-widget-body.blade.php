...
@php
    ...
    $wwManifest = json_decode(file_get_contents(public_path('vendor/whatsapp-widget/manifest.json')), true);
    $wwIconA = asset('vendor/whatsapp-widget/' . $wwManifest['resources/images/whatsapp-icon-a.svg']['file']);
    $wwIconLogo = asset('vendor/whatsapp-widget/' . $wwManifest['resources/images/whatsapp-icon-logo.svg']['file']);
    $wwAlertMp3 = asset('vendor/whatsapp-widget/' . $wwManifest['resources/midia/alert.mp3']['file']);
@endphp
                    ...
                    <a class="ww-whatsapp-button" target="_blank" data-agent="{{ $whatsappAgent->id }}"
                       ...
                       href="{{ URL::signedRoute('whatsapp-widget.redirect', ['whatsapp_agent' => $whatsappAgent->id, 'agent' => $whatsappAgent->id, 'number' => $whatsappAgent->phone, 'ref' => config('whatsapp-widget.url')]) }}">
                        <img width="60" height="60" class="ww-whatsapp-avatar ww-image"
                             ...
                             src="{{ $whatsappAgent->image_url ?? $wwIconLogo }}"/>
