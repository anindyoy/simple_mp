...
it('mengisi ulang token user yang di bawah minimum mingguan', function () {
    ...
    $this->artisan('tokens:refill-weekly')
        ->expectsOutput('Refill token mingguan selesai (minimum: 5). User diperbarui: 3.')
        ->assertExitCode(0);
...
it('tidak memperbarui user yang sudah memiliki token cukup', function () {
    ...
    $this->artisan('tokens:refill-weekly')
        ->expectsOutput('Refill token mingguan selesai (minimum: 3). User diperbarui: 0.')
        ->assertExitCode(0);
...
it('melewati user admin saat refill token', function () {
    ...
    $this->artisan('tokens:refill-weekly')
        ->expectsOutput('Refill token mingguan selesai (minimum: 4). User diperbarui: 1.')
        ->assertExitCode(0);
...
it('menggunakan nilai default 3 saat setting belum dikonfigurasi', function () {
    ...
    $this->artisan('tokens:refill-weekly')
        ->expectsOutput('Refill token mingguan selesai (minimum: 3). User diperbarui: 1.')
        ->assertExitCode(0);
